...
export async function POST(req: Request) {
  try {
    ...
    // Check if the developer provided the required environment variable
    ...
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      headers: {
        ...
        "HTTP-Referer": "https://localhost:3000", // Optional metadata tracker
      ...
      body: JSON.stringify({
        ...
        response_format: { type: "json_object" } // Demands structured JSON returned from open-weights models
      })
    });
    ...
    const processedAdSets = parsedData.generatedAdSets.map((ad: any, index: number) => {
      ...
      return {
        ...ad,
        adIndex: index + 1,
        // High performance lightweight image placeholder layer

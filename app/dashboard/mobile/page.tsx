...
import { Sparkles, Layers, Landmark, CreditCard, RefreshCw, Smartphone } from 'lucide-react';
...
export default function MobileEngineConsole() {
    ...
    try {
      const res = await fetch('/api/marketing-engine', {
        ...
        body: JSON.stringify({
          ...
          productContext: 'Mobile low-data deployment pipeline.',
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 font-sans selection:bg-slate-800">
      {/* Mobile Top Bar */}
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-lg p-4 space-y-3">
        ...
        <textarea
          ...
          placeholder="e.g., Fresh bulk farm produce delivery box, local Cape Town sourcing, R250 per box, no middle-man markup..."
        />
        <button
          ...
          <div className="space-y-3">
            {output.generatedAdSets.map((ad: any, i: number) => (
              <div key={i} className="bg-slate-900 border border-slate-800 rounded-lg p-4 space-y-3">
                ...
                {/* Low-Data image wrapper with lazy loading */}
                <img 
                  src={ad.realGeneratedImageUrl} 
                  alt="Product Asset" 
                  loading="lazy"
                  className="w-full aspect-video object-cover rounded bg-slate-950 border border-slate-850"
                />
                ...
                <div className="bg-slate-950 p-2 rounded border border-slate-850 text-[10px] text-slate-500 font-mono">
                  <span className="text-slate-400 font-bold">Visual Prompt:</span> {ad.imageGenerationPrompt}
                </div>
              </div>
            ))}
          </div>

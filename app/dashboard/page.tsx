...
import { Activity, ShieldAlert, Target, RefreshCw, Layers, Sparkles } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
          <div className="flex justify-between items-center mb-2">
            <span className="text-xs uppercase text-slate-400 font-mono">Conversion Index</span>
            <Activity className="w-4 h-4 text-emerald-400" />
          </div>
          <div className="text-2xl font-bold font-mono">5.14%</div>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
          <div className="flex justify-between items-center mb-2">
            <span className="text-xs uppercase text-slate-400 font-mono">Bounce Velocity</span>
            <ShieldAlert className="w-4 h-4 text-rose-400" />
          </div>
          <div className="text-2xl font-bold font-mono">18.6%</div>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
          <div className="flex justify-between items-center mb-2">
            <span className="text-xs uppercase text-slate-400 font-mono">Impulse Vectoring</span>
            <Target className="w-4 h-4 text-purple-400" />
          </div>
          <div className="text-2xl font-bold font-mono">OPTIMAL</div>
        </div>
      </div>

      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 mb-8">
        <h3 className="text-sm font-mono font-bold text-slate-300 uppercase tracking-wider mb-4">Intent Buyer Regional Location Metrics</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse font-mono text-xs">
            <thead>
              <tr className="border-b border-slate-800 text-slate-400">
                <th className="py-3 px-4">South African Region Node</th>
                <th className="py-3 px-4">Total Purchases Logged</th>
                <th className="py-3 px-4">Cohort Classification Matrix</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800 text-slate-300">
              {buyerDemographics.map((row, idx) => (
                <tr key={idx} className="hover:bg-slate-850">
                  <td className="py-3 px-4 font-bold">{row.location}</td>
                  <td className="py-3 px-4 text-emerald-400">{row.buyers} invoices</td>
                  <td className="py-3 px-4"><span className="px-2 py-0.5 rounded bg-slate-800 border border-slate-700 text-purple-300">{row.allocation}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
